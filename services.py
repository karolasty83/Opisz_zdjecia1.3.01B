...
class ImageDescriber(threading.Thread):
    def __init__(self, parent, api_key, image_files, prompt_text, emit_existing=True):
        ...
        self._executor = None
        self._openai_model = get_openai_model_from_config()
        self._gemini_model = get_gemini_model_from_config()
    ...
    def _describe_with_gemini(self, image_path):
        ...
        url = GEMINI_ENDPOINT_TMPL.format(model=self._gemini_model)
...
def _describe_bytes_with_openai(api_key: str, prompt_text: str, image_bytes: bytes, mime: str) -> str:
    ...
    model_name = get_openai_model_from_config()
    payload = {
        "model": model_name,
        "messages": [
            {
...
def _describe_bytes_with_gemini(api_key: str, prompt_text: str, image_bytes: bytes, mime: str) -> str:
    model_name = get_gemini_model_from_config()
    url = GEMINI_ENDPOINT_TMPL.format(model=model_name)
    params = {"key": api_key}
    headers = {"Content-Type": "application/json"}
